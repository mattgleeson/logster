...
###  of response codes found in an Apache access log.  We have
###  extended the Etsy sample clas to support an optional filtering
###  regex.
###
###  For example:
###  sudo ./logster --dry-run --output=graphite AccessLogster --parser-options='GET /search' /var/log/httpd/access_log
###
...
class AccessLogster(LogsterParser):
    ...
    def __init__(self, option_string=None):
        ...
        if option_string:
            self.filter_reg = re.compile(option_string)
        else:
            self.filter_reg = re.compile('.?')
    ...
    def parse_line(self, line):
        ...
        try:
            ...
            if not self.filter_reg.match(line):
                return
